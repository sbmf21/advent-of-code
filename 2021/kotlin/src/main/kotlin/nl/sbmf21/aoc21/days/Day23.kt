        ...
        .mapIndexed { y, it ->
            it.toCharArray().mapIndexed { x, c ->
                when (c) {
                    '#' -> Wall()
                    '.' -> Floor(Vector2(x, y))
                    in AmphipodType.types -> Floor(Vector2(x, y), Amphipod(AmphipodType.fromType(c)))
                    else -> null
                }
            }
        }.also { direGram ->
            direGram.forEach { r ->
                r.filterIsInstance(Floor::class.java).forEach { tile ->
                    tile.neighbours.addAll(listOf(Vector2(-1, 0), Vector2(0, -1), Vector2(1, 0), Vector2(0, 1))
                        .map { tile.pos + it }
...
internal class Floor(val pos: Vector2, var occupant: Amphipod? = null) : Tile {
